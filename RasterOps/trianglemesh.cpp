...
void TriangleMesh::initBuffers(
    ...
    GLuint indexBuf = 0, posBuf = 0, normBuf = 0, tcBuf = 0, tcBuf2 = 0, tangentBuf = 0, alphaBuf = 0; //Addition for blending
    ...
    //Addition for blending
    if (alpha != nullptr) {
        glGenBuffers(1, &alphaBuf);
        buffers.push_back(alphaBuf);
        glBindBuffer(GL_ARRAY_BUFFER, alphaBuf);
        glBufferData(GL_ARRAY_BUFFER, alpha->size() * sizeof(GLfloat), alpha->data(), GL_STATIC_DRAW);
    }
